...
def ConvertFileName(FileName):
    ...
    FileNameC = str(FileName.replace(
    ...
    FileNameC = FileNameC.replace('%Y', datetime.datetime.now().strftime("%Y"))
    FileNameC = FileNameC.replace(
        '%YY', datetime.datetime.now().strftime("%YY"))
    ...
    FileNameC = FileNameC.replace(
        '%-I', datetime.datetime.now().strftime("%-I"))
    ...
    if os.path.exists(values.ConfigFilePath):
        ...
        try:
            with open(values.ConfigFilePath, 'r') as ConfigFile:
                ...
                values.SystemProcessWaitValue = int(
                    JSONValues["SystemProcessWaitValue"])
                values.SystemFilesystemAndFstatWaitValue = int(
                    JSONValues["SystemFilesystemAndFstatWaitValue"])
                values.SystemDiskIOWaitValue = int(
                    JSONValues["SystemDiskIOWaitValue"])
                values.SystemProcessSummaryWaitValue = int(
                    JSONValues["SystemProcessSummaryWaitValue"])
                values.SystemLoadWaitValue = int(
                    JSONValues["SystemLoadWaitValue"])
                values.SystemFcWaitValue = int(JSONValues["SystemFcWaitValue"])
                values.SystemSocketSummaryWaitValue = int(
                    JSONValues["SystemSocketSummaryWaitValue"])
                values.SystemSocketWaitValue = int(
                    JSONValues["SystemSocketWaitValue"])
                values.SystemMemoryWaitValue = int(
                    JSONValues["SystemMemoryWaitValue"])
                values.SystemNetworkWaitValue = int(
                    JSONValues["SystemNetworkWaitValue"])
                values.SystemCoreAndCpuWaitValue = int(
                    JSONValues["SystemCoreAndCpuWaitValue"])

                
                # Loading PingPlotter targets and configuration values
                values.PingPlotterTargets = JSONValues["PingPlotterTargets"]
                values.PingPlotterWaitValue = int(
                    JSONValues["PingPlotterWaitValue"])
                ...
                values.SystemHPMStatWaitValue = int(
                    JSONValues["SystemHPMStatWaitValue"])
                values.SystemHypervisorWaitValue = int(
                    JSONValues["SystemHypervisorWaitValue"])
            ...
            logging.info("            ==> Error while loading " +
                         values.ConfigFilePath + " daemon config file !")
            logging.info(
                "                Please check the Configuration file JSON parameters, format and validity")
...
def LoadPluginConfig():
        ...
        if f_name.endswith('filebeat.conf'):
            ...
            try:
                with open(FullPathFileName, 'r') as ConfigFile:
                    ...
                    # Log
                    logging.info(
                        "            * Target file                 = " + JSONValues["TargetFile"])
                    logging.info(
                        "            * Patterns                    = " + str(JSONValues["Patterns"]))
                    logging.info(
                        "            * Multiline sparator REGEX    = " + JSONValues["MultilineSeparator"])
                    logging.info(
                    ...
                    CurrentTargetFile = ConvertFileName(
                        JSONValues["TargetFile"])

                    # Checking if file exists
                    try:
                        # Looking for corresponding file
                        OSCommand = "ls -lart " + CurrentTargetFile + \
                            " | tail -n 1 | awk '{print $NF}'"
                        CheckTargetFile = subprocess.Popen(
                            OSCommand, shell=True, stdout=subprocess.PIPE, stderr=values.devnull).stdout
                        ...
                        if os.path.exists(str(CheckTargetFile[0])):
                            ...
                            # Log
                            logging.info(
                                "            ==> Warning while loading " + f_name + " config file !")
                            logging.info(
                                "                File does not exists yet, daemon will retry later...")
                    ...
                    values.FilebeatConfigsArray[f_name] = CurrentTargetFile + ',' + JSONValues["TargetFile"] + ',' + PatternMatch + ',' + str(TargetFileLastLine) + ',' + str(
                        TargetFileInode) + ',' + str(TargetFileSize) + ',' + str(JSONValues["MultilineSeparator"] + ',' + str(JSONValues["Output"]))
            except:
                ...
                logging.info("            ==> Error while loading " +
                             f_name + " config file !")
                logging.info(
                    "                Please check JSON format and validity")
                logging.info(str(traceback.print_exc()))
                traceback.print_exc()
            # pass
        ...
        if f_name.endswith('custom.conf'):
            ...
            try:
                with open(FullPathFileName, 'r') as ConfigFile:
                    ...
                    # Log
                    logging.info(
                        "            * OS Script               = " + JSONValues["OSScript"])
                    logging.info(
                        "            * Refresh value in sec    = " + str(JSONValues["Refresh"]))
                    logging.info(
                        "            * Output                  = " + JSONValues["Output"])

                    # Storing results into FilebeatConfigsArray dictionnary
                    # ConfigString = JSONValues["OSScript"] + ',' + str(JSONValues["Refresh"]) + ',' + JSONValues["Output"]
                    values.CustomMetricsConfigsArray[f_name] = JSONValues["OSScript"] + ',' + str(
                        JSONValues["Refresh"]) + ',' + JSONValues["Output"]
            except:
                # If error, bypassing this plugin
                # Log
                logging.info("            ==> Error while loading " +
                             f_name + " config file !")
                logging.info(
                    "                Please check that target file is existing, JSON format and validity")
                logging.info(str(traceback.print_exc()))
                traceback.print_exc()
                os._exit(2)
    ...
    # Log
    logging.info(
        "            ==> All Custom Metric plugin's config files loaded !")
...
def CheckServers():
    ...
    if len(values.ElasticServersFailed) != 0:
        ...
        try:
            ...
            ElasticServerURL = values.ELKWebProtocol + '://' + \
                ElasticServer + ':' + values.ElasticPort + '/'

            # Connect and send encoded JSON message
            values.ElasticServerCnx = requests.session()
            ELKAnswer = values.ElasticServerCnx.get(
                ElasticServerURL, auth=values.ELKCreds, timeout=5, verify=values.ELKCertificate)
            ...
            logging.info("WARNING  ==> " + ElasticServer +
                         " is unavailable for Metricbeat messages")
            ...
            try:
                ...
                logging.info("INFO     ==> " + ElasticServer +
                             " is Online ! Processing Metricbeat messages on this server")
            except:
                # Nothing to do
                pass
    ...
    if len(values.FilebeatServersFailed) != 0:
        ...
        try:
            # Defining URL and credentials for web request
            values.ELKCreds = (values.ELKUsername, values.ELKPassword)
            FilebeatServerURL = values.ELKWebProtocol + '://' + \
                FilebeatServer + ':' + values.FilebeatPort + '/'

            # Connect and send encoded JSON message
            values.FilebeatServerCnx = requests.session()
            ELKAnswer = values.FilebeatServerCnx.get(
                FilebeatServerURL, auth=values.ELKCreds, timeout=5, verify=values.ELKCertificate)
            ...
            logging.info("WARNING  ==> " + FilebeatServer +
                         " is unavailable for Filebeat messages")
            ...
            try:
                ...
                logging.info("INFO     ==> " + FilebeatServer +
                             " is Online ! Processing Filebeat messages on this server")
            except:
                # Nothing to do
                pass
    ...
    if len(values.LogstashServersFailed) != 0:
        ...
        try:
            ...
            LogstashServerURL = values.ELKWebProtocol + '://' + \
                LogstashServer + ':' + values.LogstashPort + '/'

            # Connect and send encoded JSON message
            values.LogstashServerCnx = requests.session()

            ELKAnswer = values.LogstashServerCnx.get(
                LogstashServerURL, auth=values.ELKCreds, timeout=5, verify=values.ELKCertificate)
            ...
            logging.info("WARNING  ==> " + LogstashServer +
                         " is unavailable for Logstash messages")
            ...
            try:
                ...
                logging.info("INFO     ==> " + LogstashServer +
                             " is Online ! Processing Logstash messages on this server")
            except:
                # Nothing to do
                pass
    ...
    if (len(values.SendQueueArray) > values.BulkMaxSize or len(values.ElasticServersAvailable) == 0):
        LogString = "WARNING  ==>  " + \
            str(len(values.SendQueueArray)) + \
            " Metricbeat historical JSON messages queued"
        logging.info(LogString)
    if (len(values.FilebeatSendQueueArray) > values.BulkMaxSize or len(values.FilebeatServersAvailable) == 0):
        LogString = "WARNING  ==>  " + \
            str(len(values.FilebeatSendQueueArray)) + \
            " Filebeat historical JSON messages queued"
        logging.info(LogString)
    if (len(values.LogstashSendQueueArray) > values.BulkMaxSize or len(values.LogstashServersAvailable) == 0):
        LogString = "WARNING  ==>  " + \
            str(len(values.LogstashSendQueueArray)) + \
            " Logstash historical JSON messages queued"
        logging.info(LogString)
...
def CompareTimer(TimedTopic, MetricsWaitValue, CustomMetric=''):
    ...
    if TimerLatency > MetricsWaitValue:
        ...
        # Log
        LogString = "==> " + TimedTopic + " " + CustomMetric + " : New execution triggered ! (" + str(
            round(TimerLatency, 2)) + " sec elapsed for " + str(MetricsWaitValue) + " seconds wait time)."
        logging.info(LogString)
        ...
        # Log
        LogString = "  ! (" + str(round(RemainingTime, 2)) + "/" + str(MetricsWaitValue) + \
            " sec) " + TimedTopic + CustomMetric + " has NOT reached timer value."
        logging.info(LogString)
